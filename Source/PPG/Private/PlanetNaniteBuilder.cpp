...
TUniquePtr<FStaticMeshRenderData> FPlanetNaniteBuilder::CreateRenderData(bool& AbortAsync, bool RayTracingProxy)
{
	...
	else
	{
		...
		LODResource->VertexBuffers.PositionVertexBuffer.Init(TArray<FVector3f>(Mesh.Positions), false);
		...
		{
			FVector3f Normal = Mesh.Normals[i].GetUnitVector();
		...
		LODResource->Sections[0].NumTriangles = Mesh.Indices.Num() / 3;
...
void FPlanetNaniteBuilder::ApplyRenderData(UStaticMesh& StaticMesh, TUniquePtr<FStaticMeshRenderData> RenderData)
{
	VOXEL_FUNCTION_COUNTER();
...
UStaticMesh* FPlanetNaniteBuilder::CreateStaticMesh(TUniquePtr<FStaticMeshRenderData> RenderData)
{
	VOXEL_FUNCTION_COUNTER();
...
bool FPlanetNaniteBuilder::Build(FBuildData& BuildData)
#if VOXEL_ENGINE_VERSION >= 507
{
	using namespace Voxel::Nanite;
...
TVoxelArray<TVoxelArray<TUniquePtr<Voxel::Nanite::FCluster>>> FPlanetNaniteBuilder::CreatePages(
