...
IPlugin& FVoxelUtilities::GetPlugin()
{
	...
	if (!Plugin)
	{
		Plugin = IPluginManager::Get().FindPlugin("PPG");
	}
	if (!Plugin)
	{
		Plugin = IPluginManager::Get().FindPlugin("ProceduralPlanetGeneration");
	}
	checkf(Plugin.IsValid(), TEXT("Failed to resolve plugin for VoxelCore module. Tried: Voxel, VoxelCore, PPG, ProceduralPlanetGeneration"));
	return *Plugin;
}
